...
function getCommuteFactor(commuteMode: string, fuelType: string) {
  if (commuteMode === 'car') {
    if (fuelType === 'diesel') return 0.17;
    if (fuelType === 'electric') return 0.06;
    if (fuelType === 'hybrid') return 0.12;
...
export default function OnboardingClient() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1180px] px-3 py-4">
      <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section className="flex h-[calc(100dvh-8rem)] min-h-[540px] min-w-0 flex-col overflow-hidden rounded-3xl border border-border-default bg-bg-surface shadow-sm">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex min-h-0 flex-1 flex-col">
            <div className="relative min-h-0 flex-1 overflow-hidden">
              <div className="pointer-events-none absolute right-4 top-3 z-20 flex items-center gap-1.5 rounded-full border border-emerald-200 bg-white/95 px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.12em] text-emerald-700 shadow-sm backdrop-blur">
                <ArrowDown className="h-3.5 w-3.5 animate-bounce" />
                Scroll for more
              </div>

              <div className="h-full space-y-5 overflow-y-auto p-5 pt-12 [scrollbar-gutter:stable]">
                {errorMsg && (
                      ...
                      <div className="rounded-2xl border border-border-subtle bg-bg-elevated/40 p-4">
                        <div className="flex gap-3">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-sm leading-6 text-text-secondary">
                              Daily distance is converted into monthly travel emissions using
                              workday assumptions.
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="flex gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-sm leading-6 text-emerald-800">
                            ...
                            <span className="font-black">{baseline.total} kg CO₂e/month</span>.
                            Pick a target that feels repeatable.
                          </p>
                        </div>
                      </div>
                    </div>
