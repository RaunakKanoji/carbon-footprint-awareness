...
export default function OnboardingClient() {
  ...
  useEffect(() => {
    ...
    if (dbUser.name) {
      setValue('name', dbUser.name, { shouldValidate: true });
    ...
    if (dbUser.profile) {
      ...
      if (profile.city) setValue('city', profile.city, { shouldValidate: true });
      if (profile.state) setValue('state', profile.state, { shouldValidate: true });
      if (profile.country) setValue('country', profile.country, { shouldValidate: true });
      if (profile.householdSize) {
        setValue('householdSize', profile.householdSize, { shouldValidate: true });
      }

      const dietType = mapStoredDietType(profile.dietType);
      const commuteMode = mapStoredCommuteMode(profile.commuteMode);

      if (dietType) setValue('dietType', dietType, { shouldValidate: true });
      if (commuteMode) setValue('commuteMode', commuteMode, { shouldValidate: true });

      if (profile.commuteDistanceKm) {
        setValue('commuteDistance', profile.commuteDistanceKm, { shouldValidate: true });
      }

      if (profile.electricityUsageKwh) {
        setValue('monthlyKwh', profile.electricityUsageKwh, { shouldValidate: true });
      }
    }
  ...
  return (
    <div className="mx-auto w-full max-w-[1180px] px-3 py-3">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <section className="flex h-[calc(100dvh-9rem)] min-h-[500px] min-w-0 flex-col overflow-hidden rounded-3xl border border-border-default bg-bg-surface shadow-sm">
          ...
          <div className="shrink-0 border-b border-border-subtle bg-bg-elevated/35 px-5 py-3">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex min-h-0 flex-1 flex-col">
                    ...
                    <div className="flex gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm leading-6 text-text-secondary">
                          Location and household size help the app estimate electricity, commute,
                          and lifestyle assumptions more fairly.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <label className={labelClassName}>
                      ...
                      <span className={helperClassName}>
                        Used to normalize home energy impact.
                      </span>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm leading-6 text-emerald-800">
                          ...
                          <span className="font-black">{baseline.total} kg CO₂e/month</span>. Pick
                          a target that feels repeatable.
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex shrink-0 items-center justify-between border-t border-border-subtle bg-bg-surface p-5">
              <button
                type="button"
                onClick={prevStep}
